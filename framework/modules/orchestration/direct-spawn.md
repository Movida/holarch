...
| modele_cli | *(politique par profil)* | Modèle imposé à tous les profils s'il est renseigné : un **identifiant du catalogue** (`## Catalogue de modèles` de `CONFIG.md` — `opus`, `sonnet`, `fable`, `haiku`…), ou un identifiant de modèle complet, passé tel quel à l'exécuteur s'il est absent du catalogue. Laisser absent pour appliquer la politique par profil ci-dessous. |
| executeur | *(par le catalogue)* | Exécuteur imposé à toute la mission (`framework/bin/executeurs/` : `claude-code`, `fake`, `passerelle`). Absent (cas normal) : celui du fournisseur du modèle au catalogue, à défaut `claude-code`. Un nom inconnu fait échouer le lancement. |
...
| changements_regime_max | 1 | Ré-incarnations automatiques par le lanceur après un changement de régime décidé par l'instance (ligne `Profil`, `Effort` ou `Modèle` de sa fiche registre modifiée, puis hibernation volontaire — règle `ON_PLAN`), décomptées à part de `relances_max`. Ne compte que les changements posés par l'instance elle-même (ligne `Régime posé par` absente ou `soi`) — un changement posé par le mainteneur (`Régime posé par | mainteneur |`) ne décompte rien : voir « Changement de régime » ci-dessous. |
...
Chaque instance porte un **profil** : champ `Profil` de sa fiche registre, posé par son parent à `ON_SPAWN` (`conception` pour la racine, posé au bootstrap). Le lanceur en déduit le modèle et l'effort par défaut d'après la table `## Politique de modèle` de `CONFIG.md` ; à défaut, ces valeurs par défaut. Les noms de la colonne « Modèle » sont des **identifiants du catalogue** (`## Catalogue de modèles` de `CONFIG.md`), pas des noms de modèles de fournisseur : c'est le catalogue qui les traduit, au lancement, en modèle réel et en fournisseur — un même `CONFIG.md` change donc de fournisseur sans que le contrat (ce module, les `ROLE.md`, les fiches registre) n'ait un seul mot à changer. Les deux tables sont facultatives : sans elles, l'identifiant est passé tel quel à l'exécuteur `claude-code`, comportement d'avant 1.7.0.
...
**Changement de régime posé par le mainteneur.** En dehors de toute instance, une session de maintenance humaine peut aussi modifier la ligne `Profil`, `Effort` ou `Modèle` d'une fiche registre — par exemple pour corriger un régime mal calibré au spawn sans attendre que l'instance elle-même le constate. Un tel changement s'identifie par son commit : sujet `[<chemin-instance>] <résumé>` (l'instance elle-même) contre un sujet de commit humain (voir `CONTRIBUTING.md`) ; à défaut de pouvoir trancher par le seul sujet du commit, la fiche registre porte une ligne `| Régime posé par | mainteneur |` (omise ou `soi` : posé par l'instance). Un changement posé par le mainteneur n'est jamais décompté de `changements_regime_max` — ce plafond ne borne que l'auto-évaluation discrétionnaire d'une instance, pas une correction externe.

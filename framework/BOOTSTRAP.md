...
1. **Node.js et Claude Code** : le lanceur `framework/bin/holarch-spawn.js` et les garde-fous `framework/hooks/holarch-hooks.js` tournent sur Node.js (≥ 18), déjà requis pour installer le CLI (`npm install -g @anthropic-ai/claude-code`) — aucune autre dépendance. `node framework/bin/holarch-spawn.js --check-env` vérifie mécaniquement `node --version`, `git --version`, `claude --version` et une authentification active (`claude auth status`) : à lancer avant tout premier lancement réel, pour un diagnostic explicite plutôt qu'un `command not found` opaque au lancement.
